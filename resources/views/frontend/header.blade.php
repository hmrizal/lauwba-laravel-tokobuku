    ...
    <nav id="header-nav" class="navbar navbar-expand-lg px-3">
        <div class="container">
            <a class="navbar-brand d-lg-none" href="/">
                <img src="{{ asset('front-template') }}/images/main-logo.png" class="logo">
            </a>
                ...
                <div class="offcanvas-body">
                    <ul id="navbar" class="navbar-nav w-100 d-flex justify-content-between align-items-center">
                        ...
                        <ul class="list-unstyled d-lg-flex justify-content-md-between align-items-center">
                            <li class="nav-item">
                                <a class="nav-link text-uppercase ms-0" href="/about">About</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link text-uppercase ms-0" href="/contact">Contact</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link text-uppercase ms-0" href="/books">Books</a>
                            </li>
                        </ul>
                        ...
                        <ul class="list-unstyled d-lg-flex justify-content-between align-items-center">
                            <li class="nav-item">
                                @auth
                                    <a class="nav-link text-uppercase me-0" href="/logout">Logout</a>
                                @endauth
                                @guest
                                    <a class="nav-link text-uppercase me-0" href="{{ route('customer.login') }}">Sign
                                        In</a>
                                @endguest
                            </li>
                            <li class="cart-dropdown nav-item dropdown">
                                @auth
                                    <a class="nav-link text-uppercase dropdown-toggle me-0" data-bs-toggle="dropdown"
                                        href="#" role="button" aria-expanded="false">Cart(<span
                                            id="cart-count"></span>)</a>
                                    <div class="dropdown-menu dropdown-menu-end p-3">
                                        <h4 class="d-flex justify-content-between align-items-center mb-3">
                                            <span class="text-primary">Your cart</span>
                                            <span class="badge bg-primary rounded-pill"><span id="cart-count"></span></span>
                                        </h4>
                                        <ul class="list-group mb-3">
                                            @php
                                                $data = App\Models\Cart::where(['id'=> Auth::id(), 'id_nota'=>NULL])->with('dasarBuku')->get();
                                            @endphp

                                            @foreach($data as $item)
                                            <li
                                                class="list-group-item bg-transparent border-gray d-flex justify-content-between lh-sm">
                                                <div>
                                                    <h5 class="card-title mb-0">
                                                        <a href="single-element.html">{{$item->dasarBuku->judul}}</a>
                                                    </h5>
                                                </div>
                                                @if ($item->dasarBuku->diskon > 0)
                                                    <span class="text-primary">@rupiah($item->jumlah * ((100 - $item->dasarBuku->diskon) / 100) * $item->dasarBuku->harga_asli)</span>
                                                @else
                                                    <span class="text-primary">@rupiah($item->jumlah * $item->dasarBuku->harga_asli)</span>
                                                @endif
                                            </li>
                                            @endforeach
                                        </ul>
                                        <div class="d-flex flex-wrap justify-content-center">
                                            <a href="/cart" class="w-100 btn btn-dark mb-1" type="submit">View
                                                Cart</a>
                                            @if(count($data)>0)
                                            <a href="/cart/nota" class="w-100 btn btn-primary" type="submit">Go to
                                                checkout</a>
                                            @endif
                                        </div>
                                    </div>
